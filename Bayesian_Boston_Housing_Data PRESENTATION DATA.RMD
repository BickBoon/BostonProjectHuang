...
```{r}
set.seed(3)
bayesian_ridge <- function(X, y, n_iter = 10000, burn_in = 1000) {
  n <- nrow(X)
  p <- ncol(X)
  ...
  for (iter in 1:n_iter) {
    # Sample beta
    V_beta <- solve(t(X) %*% X / tau + diag(lambda, p))
    M_beta <- V_beta %*% t(X) %*% y / tau
    beta <- mvrnorm(1, M_beta, V_beta)
    
    # Sample tau
    alpha_n <- alpha + n / 2
    lambda_n <- lambda + sum((y - X %*% beta)^2) / 2
    tau <- rgamma(1, alpha_n, lambda_n)
  ...
  list(beta_samples = beta_samples[(burn_in + 1):n_iter, ],
       tau_samples = tau_samples[(burn_in + 1):n_iter])
}

# Run Bayesian Ridge
ridge_results <- bayesian_ridge(X, y)

brr_summary <- data.frame(
  Mean = apply(ridge_results$beta_samples, 2, mean),
  Median = apply(ridge_results$beta_samples, 2, median),
  Lower = apply(ridge_results$beta_samples, 2, quantile, probs = 0.025),
  Upper = apply(ridge_results$beta_samples, 2, quantile, probs = 0.975)
)
